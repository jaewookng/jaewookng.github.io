...
- **Sterile station** — a rounded brushed-steel hood under the portrait that holds the pipette and the Pro/Per switch (`--station-*` tokens; light steel on Pro, dark on Per). The cursor is a hand inside it; move onto the station and the hand picks the pipette up by its body, leave and it glides back. The switch itself navigates instantly; the theme cross-fades via the View Transitions API where supported.
